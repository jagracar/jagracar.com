var multiplePlotsSketch = function(p) {
	...
	p.setup = function() {
		...
		plot1 = new GPlot(p);
		plot1.setPos(0, 0);
		plot1.setOuterDim(0.6 * p.width, 0.4 * p.width);
		plot1.setXLim(1, 100);
		plot1.setYLim(0.1, 3);
		...
		plot2 = new GPlot(p);
		plot2.setPos(0.6 * p.width, 0);
		plot2.setOuterDim(0.4 * p.width, 0.4 * p.width);
		...
		plot3 = new GPlot(p);
		plot3.setPos(0, 0.4 * p.width);
		plot3.setOuterDim(0.4 * p.width, 0.4 * p.width);
		...
		plot4 = new GPlot(p);
		plot4.setPos(0.4 * p.width, 0.4 * p.width);
		plot4.setOuterDim(0.6 * p.width, 0.4 * p.width);
	...
	// Execute the sketch
	p.draw = function() {
		var points2, points3, points4, lastPoint, relativePos, i, index;
		...
		if (plot4.isOverBox(p.mouseX, p.mouseY)) {
			...
			relativePos = plot4.getRelativePlotPosAt(p.mouseX, p.mouseY);
